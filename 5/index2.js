...
for (let intructionIndex = numLine + 2; intructionIndex < linesSplit.length; intructionIndex++) {
    ...
    for (let k in steps) {
        k = parseInt(k);
        containers[to].push(containers[from][containers[from].length - steps.length + k]);
    }

    containers[from].splice(containers[from].length - steps.length, steps.length);
}
...
fs.writeFileSync("./test2.out", resultText.toString(), "utf8");